    ...
    def _create_mask_label(self, mix_spec, ref_spec, mask_type="IAM"):
        ...
        for r in ref_spec:
            ...
            if mask_type == "IBM":
                ...
                mask = (abs(r) / (abs(mix_spec) + eps)) * cos_theta
                mask = mask.clamp(min=0, max=1) # (0, 1)
                logmask = torch.log10(mask) #(-inf , 0)
                logmask = logmask.clamp(-5, 0) # (-inf, 0)
                mask = 5 + logmask  # (0, 5) 
        ...
        **kwargs,
        ...
        if self.loss_type != "si_snr":
            ...
            if self.mask_type != "logNPSM":
               ...
               mask_pre, flens, input_phase, input_magnitude = self.enh_model(
                    speech_mix, speech_lengths
                ) 
            ...
            if self.loss_type == "magnitude":
                ...
                logging.info(f"ideal masks is {mask_ref[0]} and its shape is {mask_ref[0].shape}")
                logging.info(f" ideal masks max value is {torch.max(mask_ref[0])} and its min value is {torch.min(mask_ref[0])}")
                logging.info(f"ideal masks type is {self.mask_type}")
