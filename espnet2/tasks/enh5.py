...
from espnet2.enh.espnet_model5 import ESPnetEnhancementModel
...
#import pysnooper
#@pysnooper.snoop()
class EnhancementTask1(AbsTask):
    ...
    def required_data_names(cls, train:  bool = True, inference: bool = False) -> Tuple[str, ...]:
        if not inference:
            retval = ("magnitude_mix", "magnitude_ref1")
        else:
            # Recognition mode
    ...
    def build_model(cls, args: argparse.Namespace) -> ESPnetEnhancementModel:
        assert check_argument_types()
        # 1. frontend (TODO) add input_dim to conf file
