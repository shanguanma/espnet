...
3.1 two channel loss of asr part:
     enh wavform -> ctc_enh_loss, att_enh_loss, y_enh
     clean wavform -> ctc_clean_loss, att_clean_loss, y_clean
3.2 using consistent loss
    ctc_loss = 0.5(ctc_enh_loss + ctc_clean_loss )
    att_enh_loss = enh_weight * enh_loss + (1- enh_weight) * att_enh_loss
    consistent_loss = KLloss(y_clean, y_enh, y_target)
    att_loss = 0.5(att_enh_loss + att_clean_loss) + consistent_loss_weight * consistent_loss
   
    loss = ctc_weigth * ctc_loss + (1- ctc_weigth) * att_loss
...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        self.consistencyLossWeight = consistencyLossWeight
        logging.info("self.consistencyLossWeight = " + str(self.consistencyLossWeight))

        self.criterionConsistencyLoss = KLLoss(vocab_size, ignore_id,
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 2a. Attention-decoder enhanced branch
        ...
        if self.enh_weight == 0.0:
            loss_enh = None
            enh_branch_att = enh_loss_att
        else:
            enh_branch_att = (1 - self.enh_weight) * enh_loss_att + self.enh_weight * loss_enh
        ...
        ys_in_lens = text_ref_lengths + 1

        # Forward decoder
        enh_decoder_out, _ = self.decoder(
             enh_encoder_out, enh_encoder_out_lens, ys_in_pad, ys_in_lens
        )
        clean_decoder_out, _ = self.decoder(
             clean_encoder_out, clean_encoder_out_lens, ys_in_pad, ys_in_lens
        ) 
        loss_consistency = self.criterionConsistencyLoss(
            clean_decoder_out, enh_decoder_out, ys_out_pad
        ) 
        loss_att = (
            enh_branch_att + clean_loss_att
        ) / 2.0 + self.consistencyLossWeight * loss_consistency

        loss_ctc = 0.5*(enh_loss_ctc + clean_loss_ctc)
        ...
        stats = dict(
            ...
            enh_branch_att=enh_branch_att.detach() if enh_branch_att is not None else None,
    ...
    def _consistent_loss(
        ...
        # 2. Compute attention consistent loss
        loss_consistency = self.criterionConsistencyLoss(
            pred_pad, pred_noisy_pad, ys_out_pad
        ) 
    def _calc_rnnt_loss(
        self,
        encoder_out: torch.Tensor,
