...
3.1 two channel loss of asr part:
     ...
     clean wavform -> ctc_clean_loss, att_clean_loss
"""
...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        (
            ...
            clean_encoder_out,
            clean_encoder_out_lens,
        ) = self.encode(
            predicted_wav, speech_mix_lengths, speech_ref1, speech_mix_lengths
        )
        ...
        if self.ctc_weight == 1.0:
            clean_loss_att, clean_acc_att, clean_cer_att, clean_wer_att = (
                None,
                None,
                None,
                None,
        ...
        if self.ctc_weight == 0.0:
            ...
            clean_loss_asr = (
                self.ctc_weight * clean_loss_ctc
                + (1 - self.ctc_weight) * clean_loss_att
            )
        ...
        if self.clean_weight == 0.0:
            ...
            loss = (
                self.clean_weight * clean_loss_asr + (1 - self.clean_weight) * enh_loss
            )
        ...
        stats = dict(
            ...
            clean_loss_att=clean_loss_att.detach()
            if clean_loss_att is not None
            else None,
            clean_loss_ctc=clean_loss_ctc.detach()
            if clean_loss_ctc is not None
            else None,
            clean_loss_asr=clean_loss_asr.detach()
            if clean_loss_asr is not None
            else None,
            acc=enh_acc_att,
            cer=enh_cer_att,
            wer=enh_wer_att,
    ...
    def inference(
        self, speech_mix: torch.Tensor, speech_mix_lengths: torch.Tensor ,speech_ref1: torch.Tensor, **kwargs,
    ...
    def inference1(
        ...
        """Enhancement + Frontend + Encoder 
        only speech_mix branch, it gets enh_wav, then two enh_wavs pass into channel attention.
        Args:
            speech_mix: (Batch, Length, ...)
            speech_mix_lengths: (Batch, )
        """
        ...
        enh_encoder_out, enh_encoder_out_lens, _, _ = self.encode(
             predicted_wav, speech_mix_lengths, predicted_wav, speech_mix_lengths
        )
        return enh_encoder_out, enh_encoder_out_lens
    ...
    def collect_feats(
        ...
        clean_branch_feats, clean_branch_feats_lengths = self._extract_feats(
            speech_clean, speech_mix_lengths
        )
        clean_branch_feats, clean_branch_feats_lengths = (
            clean_branch_feats,
            clean_branch_feats_lengths,
        )
    ...
    def encode(
        ...
        # for clean branch
        clean_feats, clean_feats_lengths = self._extract_feats(
            clean_speech, clean_speech_lengths
        )
        if self.specaug is not None and self.training:
            clean_feats, clean_feats_lengths = self.specaug(
                clean_feats, clean_feats_lengths
            )

        if self.normalize_clean_branch is not None:
            clean_feats, clean_feats_lengths = self.normalize_clean_branch(
                clean_feats, clean_feats_lengths
            )

        # for channel attention for clean branch(clean channel) and enh branch (enh channel)
        enh_feats, clean_feats = self.ca(enh_feats, clean_feats)
