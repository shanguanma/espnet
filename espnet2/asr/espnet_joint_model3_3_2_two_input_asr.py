...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        normalize_mix_branch: Optional[AbsNormalize],
        ...
        assert 0.0 <= mix_weight <= 1.0, mix_weight
        ...
        self.normalize_mix_branch = normalize_mix_branch
    ...
    def forward(
        ...
        **kwargs,
        ...
        mix_encoder_out, mix_encoder_out_lens = self.encode_mix_branch(speech_mix, speech_mix_lengths)
        ...
        if self.ctc_weight == 1.0:
            mix_loss_att, mix_acc_att, mix_cer_att, mix_wer_att = None, None, None, None
        else:
            mix_loss_att, mix_acc_att, mix_cer_att, mix_wer_att = self._calc_att_loss(
                mix_encoder_out, mix_encoder_out_lens, text_ref_all, text_ref_lengths
            )

        # 2b1. CTC branch
        if self.ctc_weight == 0.0:
            mix_loss_ctc, mix_cer_ctc = None, None
        else:
            mix_loss_ctc, mix_cer_ctc = self._calc_ctc_loss(
                mix_encoder_out, mix_encoder_out_lens, text_ref_all, text_ref_lengths
            )
        ...
        # 2d1 loss branch
        if self.ctc_weight == 0.0:
            mix_loss_asr = mix_loss_att
        elif self.ctc_weight == 1.0:
            mix_loss_asr = mix_loss_ctc
        else:
            mix_loss_asr = self.ctc_weight * mix_loss_ctc + (1 - self.ctc_weight) * mix_loss_att

        # 3 total loss
        if self.mix_weight == 0.0:
            loss = enh_loss
        elif self.mix_weight == 1.0:
            loss = mix_loss_asr
        else:
            loss = self.mix_weight * mix_loss_asr + (1 - self.mix_weight) * enh_loss        
        ...
        stats = dict(
            ...
            mix_loss_att=mix_loss_att.detach() if mix_loss_att is not None else None,
            mix_loss_ctc=mix_loss_ctc.detach() if mix_loss_ctc is not None else None,
            mix_loss_asr=mix_loss_asr.detach() if mix_loss_asr is not None else None,
            acc=enh_acc_att,
            cer=enh_cer_att,
            wer=enh_wer_att,
            cer_ctc=enh_cer_ctc,
    ...
    def collect_feats(
        ...
        mix_branch_feats, mix_branch_feats_lengths = self._extract_feats(speech_mix, speech_mix_lengths)
        mix_branch_feats, mix_branch_feats_lengths = mix_branch_feats, mix_branch_feats_lengths 
        
        return {"enh_branch_feats": enh_branch_feats,
                "enh_branch_feats_lengths":  enh_branch_feats_lengths,
                "mix_branch_feats": mix_branch_feats,
                "mix_branch_feats_lengths": mix_branch_feats_lengths,
        }
    ...
    def encode_mix_branch(
        ...
        # 3. Normalization for feature: e.g. Global-CMVN, Utterance-CMVN
        #logging.info(f"in the encode function, input of self.normalize is {feats} and its shape is {feats.shape}")
        if self.normalize_mix_branch is not None:
            feats, feats_lengths = self.normalize_mix_branch(feats, feats_lengths)
        #logging.info(f"in the encode function, output of self.normalize is {feats} and its shape is {feats.shape}")
        # 4. Forward encoder
        # feats: (Batch, Length, Dim)
        # -> encoder_out: (Batch, Length2, Dim2)
