...
###        return encoder_out, encoder_out_lens
    ...
    def inference(
        ...
        **kwargs,
        ...
        #    speech_mix, speech_mix_lengths,
        #)
        # wav -> stft -> magnitude-> utt_mvn -> rnn -> dropout->linear-> nonlinear -> mask  * _> predicted_complex_stft->istft->predicted_wav
        # wav ->                                                      stft  -> complex_stft
        input_spectrum, flens = self.enh_model.stft(speech_mix, speech_mix_lengths)
        import torchaudio
        ...
        input_magnitude_mvn, flens = utt_mvn(input_magnitude, flens)
        x, flens, _ = self.enh_model.rnn(input_magnitude_mvn, flens) 
        x = self.enh_model.dropout(x) 
        masks = []
        for linear in self.enh_model.linear:
            y = linear(x)
            y = self.enh_model.nonlinear(y)
            masks.append(y) 
        from collections import OrderedDict
        mask_pre = OrderedDict(
        ...
        # asr part
        # predicted_wav -> log-mel -> utt_mvn -> encoder
        feats, feats_lengths = self._extract_feats(predicted_wav, speech_mix_lengths)
        feats, feats_lengths = utt_mvn(feats, feats_lengths) 
