...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        self.linear_in = torch.nn.Linear(num_bin, embed_dims)
        self.self_att = torch.nn.MultiheadAttention(embed_dims, num_heads)
        self.q = torch.nn.Linear(embed_dims, embed_dims)
        self.k = torch.nn.Linear(embed_dims, embed_dims)
        self.v = torch.nn.Linear(embed_dims, embed_dims)
        self.linear_out = torch.nn.Linear(embed_dims, num_bin)
    ...
    def encode_enh_branch(
        ...
        if self.self_att is not None:
             feats = self.linear_in(feats)
             feats_q = self.q(feats)
             feats_k = self.k(feats)
             feats_v = self.v(feats)
             attn_output,_ = self.self_att(feats_q, feats_k, feats_v)
             feats = self.linear_out(attn_output)
