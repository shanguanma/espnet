...
2. load pretrain and fixed se model and train asr from scratch.
3. two input of asr part: 
     enh wavform
     noisy wavform
     enh wavform -> log-mel_enh     _> stack -> global_cmvn ->  encoder -> ctc_enh_loss, att_enh_loss
     noisy wavform -> log-mel_noisy
...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        self.linear = torch.nn.Linear(in_features=160, out_features=80, bias=True)
    ...
    def forward(
        ...
        **kwargs,
        ...
        encoder_out, encoder_out_lens = self.encode(predicted_wav, speech_mix, speech_mix_lengths)
    ...
    def collect_feats(
        ...
        predicted_spectrum = speech_mix_spectrum_complex * mask_pre['spk1']
        predicted_wav = self.enh_model.stft.inverse(predicted_spectrum , speech_mix_lengths)[0]
        feats_enh, feats_enh_lengths = self._extract_feats(predicted_wav, speech_mix_lengths)
        feats_mix, feats_mix_lengths = self._extract_feats(speech_mix, speech_mix_lengths)
        feats = torch.cat([feats_enh,feats_mix], dim=2)
        feats = self.linear(feats)
        feats, feats_lengths = feats, feats_enh_lengths
        return {"feats": feats, "feats_lengths": feats_lengths}

    def encode(
        self, speech_enh: torch.Tensor,speech_mix: torch.Tensor, speech_lengths: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Frontend + Encoder. Note that this method is used by asr_inference.py

        Args:
            speech_enh: (Batch, samples)
            speech_mix: (Batch, samples)
            speech_lengths: (Batch, )
        """
        # 1. Extract feats
        feats_enh, feats_enh_lengths = self._extract_feats(speech_enh, speech_lengths)
        feats_mix, feats_mix_lengths = self._extract_feats(speech_mix, speech_lengths)
        feats_lengths = feats_enh_lengths
        feats = torch.cat([feats_enh,feats_mix], dim=2)
        feats = self.linear(feats)
        ...
        logging.info(f" ASR module encoder input is feats is {feats}, and its shape is {feats.shape}")
        encoder_out, encoder_out_lens, _ = self.encoder(feats, feats_lengths)

        assert encoder_out.size(0) == speech_mix.size(0), (
            encoder_out.size(),
            speech_mix.size(0),
        )
