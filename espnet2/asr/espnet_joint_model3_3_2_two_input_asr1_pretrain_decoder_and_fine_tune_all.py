...
2. first pretrain decoder only text(it may be from pair data or it may be from extral text), then train all with pair data.
...
"""
### make prepared batch for only text data 
def loadTextOnlyData(textOnlyJson, bs):
    with open(textOnlyJson, 'rb') as f:
        data = json.load(f)['utts']
    sorted_data = sorted(data.items(), key=lambda data: int(len(data[1]['output'][0]['tokenid'].split())), reverse=True)
    sorted_tran = [dat[1]['output'][0]['tokenid'].encode('ascii','ignore').split() for dat in sorted_data]
    sorted_tran = [np.fromiter(map(int, sorted_tran[i]), dtype=np.int64) for i in range(len(sorted_tran))]
    logging.warning('The first sentence has size = ' + str(sorted_tran[0].shape) + ', the last sentence has size = ' + str(sorted_tran[-1].shape))
    logging.warning('Content of the 10h sentence from tail = ' + str(sorted_tran[-9]))
    minibatchs = []
    start = 0
    while True:
        end = min(len(sorted_tran), start + bs)
        minibatch = sorted_tran[start:end]
        minibatchs.append(minibatch)
        start = end
        if start >= len(sorted_tran):
            break
    random.shuffle(minibatchs)
    return minibatchs
...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        numTextOnlyBatchsPerNormalData: int=10,
        sym_space: str = "<space>",
        sym_blank: str = "<blank>",
        num_spk: int = 1,
    ):
        ...
        # related pre-train decoder
        self.listSortedTextOnly = loadTextOnlyData(textOnlyJson, 30)
        self.numTextOnlyBatchsPerNormalData = numTextOnlyBatchsPerNormalData
        self.iterTextOnly = 0 #Iterator to iterate through text-only data
    ...
    def forward(
        ...
        **kwargs,
        ...
        enh_encoder_out, enh_encoder_out_lens = self.encode_enh_branch(predicted_wav, speech_mix_lengths)
        ...
        #  only text to train decoder
        if self.pretrain_decoder:
            logging.info(f"self.pretrain_decoder is {self.pretrain_decoder}")
            totalLostText = 0
            totalAccTextOnly = 0
            totalLength = 0
            from espnet.nets.pytorch_backend.nets_utils import pad_list

            for _ in range(self.numTextOnlyBatchsPerNormalData):
                if self.iterTextOnly >= len(self.listSortedTextOnly):
                    logging.warning('md: We have just finished one opoch of text-only data')
                    self.iterTextOnly = 0
                    random.shuffle(self.listSortedTextOnly)
                batch_textOnly = self.listSortedTextOnly[self.iterTextOnly]
                avgLength = np.mean([len(x) for x in batch_textOnly])
                totalLength = totalLength + avgLength

                batch_pad = pad_list([torch.from_numpy(y).long() for y in batch_textOnly], self.ignore_id)
                self.iterTextOnly = self.iterTextOnly + 1

                ys_in_pad_text, ys_out_pad_text = add_sos_eos(batch_pad,self.sos, self.eos, self.ignore_id)
                ys_in_pad_text = ys_in_pad_text.cuda() # (B,L)
                logging.info(f"ys_in_pad_text shape is {ys_in_pad_text.shape} its ")
                ys_out_pad_text = ys_out_pad_text.cuda() # (B,L)
                logging.info(f"ys_out_pad_text shape is {ys_out_pad_text.shape}")
                ys_mask = self.target_mask(ys_in_pad_text) # (B,L,L)
                logging.info(f"ys_mask shape is {ys_mask.shape}")

                pred_pad, pred_mask = self.decoder.forward_text(ys_in_pad_text,ys_mask)

                lossTextOnly = self.criterion_att(pred_pad, ys_out_pad_text)
                accTextOnly = th_accuracy(pred_pad.view(-1, self.vocab_size), ys_out_pad_text,
                               ignore_label=self.ignore_id)
                totalLostText = totalLostText + lossTextOnly
                totalAccTextOnly = totalAccTextOnly + accTextOnly

            totalLostText = totalLostText/self.numTextOnlyBatchsPerNormalData
            totalAccTextOnly = totalAccTextOnly/self.numTextOnlyBatchsPerNormalData
            totalLength = totalLength/self.numTextOnlyBatchsPerNormalData
            if totalLength == 0:
                totalLength = 1
            logging.warning(f"md: average text-only loss of {str(self.numTextOnlyBatchsPerNormalData)}\n"
                            f" batches = {str(totalLostText.cpu().detach().numpy())}\n"
                            f" with avgLength text = {str(totalLength)}")

            loss_att = totalLostText
            loss= loss_att
        ...
        ## fine-tune all the network(contains ctc, encoder, pre-trained decoder)
        else:
            logging.info(f"self.pretrain_decoder is {self.pretrain_decoder}")
            ...
            if self.ctc_weight == 1.0:
                ...
                enh_loss_att, enh_acc_att, enh_cer_att, enh_wer_att = self._calc_att_loss(
            ...
            if self.enh_weight == 0.0:
                loss_enh = None
                enh_loss = enh_loss_asr
            else:
                enh_loss = (1 - self.enh_weight) * enh_loss_asr + self.enh_weight * loss_enh
  
            # 2a1. Attention-decoder branch
            if self.ctc_weight == 1.0:
                clean_loss_att, clean_acc_att, clean_cer_att, clean_wer_att = None, None, None, None
            else:
                clean_loss_att, clean_acc_att, clean_cer_att, clean_wer_att = self._calc_att_loss(
            ...
            if self.clean_weight == 0.0:
                ...
                loss = self.clean_weight * clean_loss_asr + (1 - self.clean_weight) * enh_loss        
        ...
        stats = dict(
            ...
            enh_loss=enh_loss.detach() if enh_loss is not None else None,
    ...
    def inference(
        ...
        **kwargs,
        ...
        encoder_out, encoder_out_lens = self.encode_enh_branch(predicted_wav, speech_mix_lengths)
