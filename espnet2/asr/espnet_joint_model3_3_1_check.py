...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 1c (optional)  store predicted wavform
        tmp_path="exp_rats_Ach_waveform_8k_joint_enh_asr_1/joint_conformer_kernel15_small_load_fixed_pretrain_se_model_1_2_1_again_raw_bpe/enhanced_wavs"
        tmp_path=Path(tmp_path)
        MAX_INT16 = np.iinfo(np.int16).max 
        with SoundScpWriter(tmp_path, tmp_path / "wav.scp", dtype=np.int16) as writer:
            speech_mix_numpy = speech_mix.cpu().numpy()
            speech_mix_int16 = (speech_mix_numpy * MAX_INT16).astype(np.int16)
            for i, j in zip(uttid, speech_mix_int16):
                writer[i] = 8000, j
        # 1d, enhanced wavform pass asr extract feature and asr encoder
    ...
    def inference(
        ...
        **kwargs,
        ...
        logging.info(f"in the forward fucntion in espnet_joint_model3_3_1.py,predicted_wav is {predicted_wav} and its shape is {predicted_wav.shape} and its dtype is {predicted_wav.dtype}")
        speech_mix = predicted_wav
        # 1c (optional)  store predicted wav 
        tmp_path="exp_rats_Ach_waveform_8k_joint_enh_asr_1/joint_conformer_kernel15_small_load_fixed_pretrain_se_model_1_2_1_check_raw_bpe/enhanced_testset_wavs"
        tmp_path=Path(tmp_path)
        MAX_INT16 = np.iinfo(np.int16).max
        with SoundScpWriter(tmp_path, tmp_path / "wav.scp", dtype=np.int16) as writer:
            speech_mix_numpy = speech_mix.cpu().numpy()
    ...
    def collect_feats(
        ...
        with torch.no_grad():
            ...
            speech_mix = predicted_wav
            feats, feats_lengths = _extract_feats(speech_mix, speech_mix_lengths)
