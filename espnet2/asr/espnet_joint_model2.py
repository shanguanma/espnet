...
"""
# MD : 2021-2-19
This script serves the speech enhancement(se) with asr model. The input of  se is the waveform , and output of se is the waveform.
I don't check it, currently I don't use it.
"""
...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        logging.info(f"in the forward fucntion in espnet_joint_model2.py, text_ref1 is {text_ref1}, its shape is {text_ref1.shape}")
        ...
        logging.info(f"speech_pre shape is {speech_pre.shape}")
        #logging.info(f"enhanced model output waveform shape in espnet_joint_model.py  is {speech_pre[0].shape}") 
        ...
        logging.info(f"in the forward fucntion in espnet_joint_model2.py , speech_pre_all is {speech_pre_all} its shape is {speech_pre_all.shape}")

        speech_pre_lengths = torch.stack(
            [speech_mix_lengths], dim=1
        ).view(-1)
        logging.info(f"in the forward fucntion in espnet_joint_model2.py, speech_pre_lengths is {speech_pre_lengths}")

        text_ref_all = torch.stack([text_ref1], dim=1).view(
            batch_size * 1, -1
        )
        logging.info(f"in the forward fucntion in espnet_joint_model2.py, text_ref_all is {text_ref_all} its shape is {text_ref_all.shape}")

        text_ref_lengths = torch.stack(
            [text_ref1_lengths], dim=1
        ).view(-1)
        logging.info(f"in the forward fucntion in espnet_joint_model2.py, text_ref_lengths is {text_ref_lengths}")
    ...
    def forward_enh(
        ...
        **kwargs,
        ...
        if self.loss_type == "magnitude3":
            # refrence: https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module
            # its has a variable "training", 
            # Boolean represents whether this module is in training or evaluation mode.
            # (TODO)md, why it is set to false, because it generated waveform , network don't update parameter
            self.enh_model.training = False
            speech_pre, speech_lenghts, *__ = self.enh_model.forward_rawwav(
                speech_mix, speech_lengths
            )
