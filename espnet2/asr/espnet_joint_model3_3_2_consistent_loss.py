...
3. three input of asr part: 
     enh wavform
     clean wavform
     noisy wavform
3.1 three channel loss of asr part:
     enh wavform -> ctc_enh_loss, att_enh_loss , enh_asr = 0.3* ctc_enh_loss + 0.7* att_enh_loss
     clean wavform -> ctc_clean_loss, att_clean_loss, clean_asr = 0.3* ctc_clean_loss+0.7*att_clean_loss
     noisy wavform -> ctc_noisy_loss, att_noisy_loss, noisy_asr = 0.3* ctc_noisy_loss + 0.7 * att_noisy_loss
     asr_loss = 1/3(enh_asr + clean_asr + noisy_asr)
3.2 consistent loss
    consistent_loss = 1/2[KL(y_noisy, y_clean) + KL(y_enh,y_clean)]
3.3 total loss = s * asr_loss + (1-s) * consistent_loss
"""
