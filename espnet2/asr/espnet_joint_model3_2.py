...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        logging.info(f"in the forward fucntion in espnet_joint_model3.py, text_ref1 is {text_ref1}, its shape is {text_ref1.shape}")
        ...
        """
        loss_enh, perm, _, tf_length,mask_pre  = self.forward_enh( 
            speech_mix,
            speech_mix_lengths,
            uttid = uttid,
        ...
        #input_magnitude, input_phase = torchaudio.functional.magphase(speech_mix_spectrum)
        #predicted_spectrum = speech_mix_spectrum_complex * mask_pre['spk1']
        #predicted_magnitude = abs(predicted_spectrum)
        #predicted_phase =  predicted_spectrum / (predicted_magnitude + + 10e-12) 

        # for getting predicted magnitude
        # torch.stft
        #iput_spectrum_file="/home4/md510/w2020/espnet-recipe/enhancement_on_espnet2/exp_8k_1/enh_train_enh_tf_mask_magnitude3_same_data_Ach_train_8k_8k_raw/enhanced_data_Ach_train_8k_10/output_of_stft_file.pkl"
        # librosa.stft
        input_spectrum_file="/home4/md510/w2020/espnet-recipe/enhancement_on_espnet2/exp_8k_1/enh_train_enh_tf_mask_magnitude3_same_data_Ach_train_8k_8k_raw/enhanced_data_Ach_train_8k_10/librosa_stft_mat_file.pkl"
        with open(input_spectrum_file, "rb") as f:
            input_spectrum_mat_1 = pickle.load(f)
        if uttid[0] in input_spectrum_mat_1:
            input_spectrum_mat_2 = input_spectrum_mat_1[uttid[0]]
        masks = mask_pre["spk1"].squeeze(0).cpu().data.numpy()
        masks = masks.transpose() # freq x time  
        #input_spectrum_mat_2 = input_spectrum_mat_2.to(speech_mix_lengths.device)
        #speech_mix_spectrum = input_spectrum_mat_2
        #speech_mix_spectrum_complex = ComplexTensor(speech_mix_spectrum[..., 0], speech_mix_spectrum[..., 1])
        logging.info(f"in the espnet_joint_model3_2.py forward,input_spectrum_mat_2  is from librosa_stft_mat_file.pkl its is {input_spectrum_mat_2} and its shape is {input_spectrum_mat_2.shape}") 
        predicted_spectrum = input_spectrum_mat_2 * masks
        logging.info(f"in the espnet_joint_model3_2.py forward,predicted_spectrum is {predicted_spectrum} and its shape is {predicted_spectrum.shape}")
        predicted_magnitude_1 = abs(predicted_spectrum)
        logging.info(f"in the espnet_joint_model3_2.py forward,predicted_magnitude_1 is {predicted_magnitude_1} its shape is {predicted_magnitude_1.shape}")
        predicted_magnitude_tensor = torch.tensor(predicted_magnitude_1,dtype=torch.float32, device=speech_mix_lengths.device)
        predicted_magnitude = predicted_magnitude_tensor.permute(1,0).unsqueeze(0)
        
        logging.info(f"in the espnet_joint_model3_2.py forward, mask is {masks} its shape is {masks.shape}")
        logging.info(f"in the espnet_joint_model3_2.py forward,predicted_magnitude is {predicted_magnitude} its shape is {predicted_magnitude.shape}")
        #logging.info(f"in the espnet_joint_model3_2.py forward, predicted_magnitude is {predicted_magnitude}")
        ...
        #predicted_magnitude = predicted_magnitude[0]
        #logging.info(f"enhanced model output predicted_magnitude is {predicted_magnitude} its shape is {predicted_magnitude.shape}, ")
        ...
        #    batch_size * 1, -1
        #)
        #logging.info(f"in the forward fucntion in espnet_joint_model3.py, text_ref_all is {text_ref_all} its shape is {text_ref_all.shape}")

        #text_ref_lengths = torch.stack(
        #    [text_ref1_lengths], dim=1
        #).view(-1)
        #logging.info(f"in the forward fucntion in espnet_joint_model3.py, text_ref_lengths is {text_ref_lengths}")

        # 1. Encoder
        #logging.info(f"in the forward fucntion in espnet_joint_model3_1.py,predicted_magnitude is {predicted_magnitude} its shape is {predicted_magnitude.shape}")
        #logging.info(f"in the forward fucntion in espnet_joint_model3_1.py,predicted_phase is {predicted_phase} and its shape is {predicted_phase.shape}")
