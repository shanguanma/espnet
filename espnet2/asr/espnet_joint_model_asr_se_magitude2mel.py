...
### compared to espnet2/asr/espnet_joint_model_asr_se_magitude2mel.py 
### it output of se is magnitude spectrum, magnitude_spectrum = abs(noisy_complex_stft*mask)
magnitude_spectrum2mel(magnitude(stft_512_256), feature dim:  257->80
...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # case2 predicted magnitude
        predicted_spectrum = speech_mix_spectrum_complex * mask_pre['spk1']
        logging.info(f"predicted_spectrum = speech_mix_spectrum_complex * mask_pre['spk1'] requires_grad is {predicted_spectrum.requires_grad}")
        predicted_magnitude = abs(predicted_spectrum)
        logging.info(f"predicted_magnitude = abs(predicted_spectrum) requires_grad is {predicted_magnitude.requires_grad}")
        encoder_out, encoder_out_lens = self.encode(predicted_magnitude, flens)
        
        # case3 predicted magnitude
        #predicted_magnitude = abs(speech_mix_spectrum_complex) * mask_pre['spk1']
        #encoder_out, encoder_out_lens = self.encode(predicted_magnitude, flens)
