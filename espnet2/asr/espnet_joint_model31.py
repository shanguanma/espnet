...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 1. Encoder
        logging.info(f"in the forward fucntion in espnet_joint_model31.py,predicted_magnitude is {predicted_magnitude} its shape is {predicted_magnitude.shape}")
        logging.info(f"in the forward fucntion in espnet_joint_model31.py,predicted_phase is {predicted_phase} and its shape is {predicted_phase.shape}")
        logging.info(f"in the forward fucntion in espnet_joint_model31.py, tf_length is {tf_length}")
        ...
        logging.info(f"in the espnet_joint_model31.py, total loss is {loss}, loss_att is {loss_att} loss_ctc is {loss_ctc} and  enh_loss is {loss_enh}")
    ...
    def inference(
        ...
        **kwargs,
        ...
        # 1. Encoder
        logging.info(f"in the inference fucntion in espnet_joint_model31.py,predicted_magnitude is {predicted_magnitude} its shape is {predicted_magnitude[0].shape}")
        logging.info(f"in the inference fucntion in espnet_joint_model31.py, tf_length is {tf_length}")
    ...
    def collect_feats(
        ...
        _, perm, _, tf_length,mask_pre  = self.forward_enh(
            speech_mix,
            speech_mix_lengths,
            speech_ref1=speech_ref1,
        )
        ...
        predicted_magnitude = abs(predicted_spectrum)
        # 1. Extract feats
        feats, feats_lengths = self._extract_feats(predicted_magnitude, flens)
    ...
    def encode(
        ...
        #### 2021-4-22, in order to plot enhanced magnitude spectrum, and log-mel spectrum, first i stored them.
        import numpy as np
        np.savez("./enh_magnitude_and_magnitude2log_mel_31.npz", magnitude=speech, log_mel=feats)
