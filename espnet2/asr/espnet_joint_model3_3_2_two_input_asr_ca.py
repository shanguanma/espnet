...
from espnet2.layers.channelattention1 import ChannelSelfAttention1
from espnet2.layers.channelattention import ChannelSelfAttention


"""
### this script is modified from espnet_joint_model3_3_2_two_input_asr1_ca.py  
# MD : 2021-3-26
...
3.1 two channel loss of asr part:
     ...
     noisy wavform -> ctc_noisy_loss, att_noisy_loss
"""
...
class ESPnetEnhASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        assert 0.0 <= noisy_weight <= 1.0, noisy_weight
        ...
        self.ca = {
            "channelselfatt1": ChannelSelfAttention1(
                feats_dim=feats_dim, dim=dim, residual=residual
            ),
            "channelselfatt": ChannelSelfAttention(
                num_channels=num_channels, residual=residual
            ),
        }[ca_type]
    ...
    def forward(
        ...
        **kwargs,
        ...
        speech_mix_spectrum_complex = ComplexTensor(
            speech_mix_spectrum[..., 0], speech_mix_spectrum[..., 1]
        )
        predicted_spectrum = speech_mix_spectrum_complex * mask_pre["spk1"]
        predicted_wav = self.enh_model.stft.inverse(
            predicted_spectrum, speech_mix_lengths
        )[0]
        # 1, enhanced wavform pass asr extract feature and asr encoder
        (
            enh_encoder_out,
            enh_encoder_out_lens,
            noisy_encoder_out,
            noisy_encoder_out_lens,
        ) = self.encode(
            predicted_wav, speech_mix_lengths, speech_mix, speech_mix_lengths
        )
        ...
        if self.ctc_weight == 0.0:
            ...
            enh_loss_asr = (
                self.ctc_weight * enh_loss_ctc + (1 - self.ctc_weight) * enh_loss_att
            )
        ...
        if self.ctc_weight == 1.0:
            noisy_loss_att, noisy_acc_att, noisy_cer_att, noisy_wer_att = (
                None,
                None,
                None,
                None,
        ...
        # 2b1. CTC branch
        if self.ctc_weight == 0.0:
            noisy_loss_ctc, noisy_cer_ctc = None, None
        else:
            noisy_loss_ctc, noisy_cer_ctc = self._calc_ctc_loss(
                noisy_encoder_out,
                noisy_encoder_out_lens,
                text_ref_all,
                text_ref_lengths,
        ...
        # 2d1 loss branch
        if self.ctc_weight == 0.0:
            noisy_loss_asr = noisy_loss_att
        elif self.ctc_weight == 1.0:
            noisy_loss_asr = noisy_loss_ctc
        else:
            noisy_loss_asr = (
                self.ctc_weight * noisy_loss_ctc
                + (1 - self.ctc_weight) * noisy_loss_att
            )

        # 3 total loss
        if self.noisy_weight == 0.0:
            loss = enh_loss
        elif self.noisy_weight == 1.0:
            loss = noisy_loss_asr
        else:
            loss = (
                self.noisy_weight * noisy_loss_asr + (1 - self.noisy_weight) * enh_loss
            )
        ...
        stats = dict(
            ...
            noisy_loss_att=noisy_loss_att.detach()
            if noisy_loss_att is not None
            else None,
            noisy_loss_ctc=noisy_loss_ctc.detach()
            if noisy_loss_ctc is not None
            else None,
            noisy_loss_asr=noisy_loss_asr.detach()
            if noisy_loss_asr is not None
            else None,
            acc=enh_acc_att,
            cer=enh_cer_att,
            wer=enh_wer_att,
    ...
    def inference(
        ...
        _, tf_length, mask_pre = self.enh_model(speech_mix, speech_mix_lengths,)
        speech_mix_spectrum, flens = self.enh_model.stft(speech_mix, speech_mix_lengths)
        speech_mix_spectrum_complex = ComplexTensor(
            speech_mix_spectrum[..., 0], speech_mix_spectrum[..., 1]
        )
        ...
        enh_encoder_out, enh_encoder_out_lens, _, _ = self.encode(
             predicted_wav, speech_mix_lengths, speech_mix, speech_mix_lengths
        )
        return enh_encoder_out, enh_encoder_out_lens
    ...
    def collect_feats(
        ...
        enh_branch_feats, enh_branch_feats_lengths = self._extract_feats(
            predicted_wav, speech_mix_lengths
        )
        # enh_feats, enh_feats_lengths = enh_feats, enh_feats_lengths
        enh_branch_feats, enh_branch_feats_lengths = (
            enh_branch_feats,
            enh_branch_feats_lengths,
        )

        # for mix speech branch
        noisy_branch_feats, noisy_branch_feats_lengths = self._extract_feats(
            speech_mix, speech_mix_lengths
        )
        noisy_branch_feats, noisy_branch_feats_lengths = (
            noisy_branch_feats,
            noisy_branch_feats_lengths,
        )
    ...
    def encode(
        self,
        enh_speech: torch.Tensor,
        enh_speech_lengths: torch.Tensor,
        noisy_speech: torch.Tensor,
        noisy_speech_lengths: torch.Tensor,
    ):
        # for enh branch
        enh_feats, enh_feats_lengths = self._extract_feats(
            enh_speech, enh_speech_lengths
        )
        if self.specaug is not None and self.training:
            enh_feats, enh_feats_lengths = self.specaug(enh_feats, enh_feats_lengths)

        if self.normalize_enh_branch is not None:
            enh_feats, enh_feats_lengths = self.normalize_enh_branch(
                enh_feats, enh_feats_lengths
            )
        # for noisy branch
        noisy_feats, noisy_feats_lengths = self._extract_feats(
            noisy_speech, noisy_speech_lengths
        )
        if self.specaug is not None and self.training:
            noisy_feats, noisy_feats_lengths = self.specaug(
                noisy_feats, noisy_feats_lengths
            )

        if self.normalize_noisy_branch is not None:
            noisy_feats, noisy_feats_lengths = self.normalize_noisy_branch(
                noisy_feats, noisy_feats_lengths
            )

        # for channel attention for noisy branch(noisy channel) and enh branch (enh channel)
        enh_feats, noisy_feats = self.ca(enh_feats, noisy_feats)
        enh_encoder_out, enh_encoder_out_lens, _ = self.encoder(
            enh_feats, enh_feats_lengths
        )
        noisy_encoder_out, noisy_encoder_out_lens, _ = self.encoder(
            noisy_feats, noisy_feats_lengths
        )
    ...
    def forward_enh(
        ...
        **kwargs,
        ...
        input_spectrum, flens = self.enh_model.stft(
            speech_ref, speech_lengths
        )  # it need to check speech_lengths
    ...
    def _permutation_loss3(ref, inf, magnitude_lengths, perm=None):
        ...
        def loss():
            ...
            # logging.info(f"")
            input_size_ = torch.tensor(
                input_size, dtype=torch.float32, device=inf_magnitude.device
            )
