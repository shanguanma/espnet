...
def inference(
    ...
    num_spk = enh_model.separator.num_spk

    # 3. Build data-iterator
    loader = EnhancementTask1.build_streaming_iterator(
        data_path_and_name_and_type,
        dtype=dtype,
        batch_size=batch_size,
        key_file=key_file,
        num_workers=num_workers,
        preprocess_fn=EnhancementTask1.build_preprocess_fn(enh_train_args, False),
        collate_fn=EnhancementTask1.build_collate_fn(enh_train_args, False),
    ...
    for keys, batch in loader:
        ...
        with torch.no_grad():
            ...
            logging.info(f"enhanced model input uttid is {keys} ,input magnitude is {magnitude_mix_mvn} its shape is {magnitude_mix_mvn.shape},\
                output mask is {masks[0]} its shape is {masks[0].shape}") 
            ...
            logging.info(f"predicted_complex feature is {predicted_complex}")
            # b5 
            samps = librosa.istft(
