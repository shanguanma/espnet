...
from espnet2.tasks.enh_asr2_3_2_two_input_asr1_ca import ASRTask
...
class Speech2Text:
    ...
    @torch.no_grad()
    def __call__(
        self, speech_mix: Union[torch.Tensor, np.ndarray], speech_ref1: Union[torch.Tensor, np.ndarray],
    ) -> List[List[Tuple[float, Optional[str], List[str], List[int], Hypothesis]]]:
        """Inference

        Args:
            speech_mix: Input noisy speech data 
            speech_ref1: input clean speech data
        Returns:
        ...
        assert check_argument_types()
        speech = speech_mix
        clean_speech = speech_ref1
        # Input as audio signal
        if isinstance(speech, np.ndarray):
            speech = torch.tensor(speech)

        if isinstance(clean_speech, np.ndarray):
            clean_speech = torch.tensor(clean_speech)

        # data: (Nsamples,) -> (1, Nsamples)
        speech = speech.unsqueeze(0).to(getattr(torch, self.dtype))
        clean_speech = clean_speech.unsqueeze(0).to(getattr(torch, self.dtype))
        # lenghts: (1,)
        lengths = speech.new_full([1], dtype=torch.long, fill_value=speech.size(1))
        batch = {"speech": speech,"clean_speech": clean_speech, "speech_lengths": lengths}
        ...
        enc, _ = self.joint_model.inference(batch['speech'], batch['speech_lengths'], batch["clean_speech"]) 
