...
from espnet2.tasks.enh4 import EnhancementTask
...
def inference(
    ...
    for keys, batch in loader:
        ...
        with torch.no_grad():
            ...
            logging.info(f"in the inference function , batch is {batch}")
            # b. Forward Enhancement Frontend
            waves, _, _ = enh_model.enh_model.forward_rawwav(
                batch["speech_mix"], batch["speech_mix_lengths"]
            )
            logging.info(f"input noisy wave is {batch['speech_mix']}  and its ilens is {batch['speech_mix_lengths']}")
            logging.info(f"waves is {waves} and its shape is {waves[0].shape}")
