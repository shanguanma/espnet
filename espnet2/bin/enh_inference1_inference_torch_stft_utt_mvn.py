...
from espnet2.tasks.enh1_utt_mvn import EnhancementTask
...
def inference(
    ...
    for keys, batch in loader:
        ...
        with torch.no_grad():
            ...
            ######### librosa.stft version, pesq is 2.49
            ...
            # torch version stft, 2021-3-24
            input_spectrum, flen = enh_model.enh_model.stft(batch["speech_mix"], batch["speech_mix_lengths"])
            input_spectrum = ComplexTensor(input_spectrum[..., 0], input_spectrum[..., 1])
            predicted_spectrum = input_spectrum * masks['spk1'] # B x T x F x 2
            samps,ilen =enh_model.enh_model.stft.inverse(predicted_spectrum, batch["speech_mix_lengths"]) # range is -1,1
            samps = samps.squeeze(0).numpy() # (sample_point,)
            samps_int16 = (samps * MAX_INT16).astype(np.int16) 
            import scipy.io.wavfile as wf
